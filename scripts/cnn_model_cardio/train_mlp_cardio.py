...
from scripts.cnn_model_cardio.create_mlp_model import create_mlp_model
...
def train_mlp_cardio():
    ...
    train_mlb = pd.DataFrame(mlb.fit_transform(train_labels), columns=mlb.classes_)
    val_mlb = pd.DataFrame(mlb.fit_transform(val_labels), columns=mlb.classes_)
    test_mlb = pd.DataFrame(mlb.fit_transform(test_labels), columns=mlb.classes_)
    ...
    print(f"\nEvaluare pe test - Loss: {loss:.4f} | Acc: {acc:.4f} | Prec: {prec:.4f} | Rec: {rec:.4f}")
    ...
    # # === 13. Salvare model
    # model.save("models/mlp_cardio_risk.h5")
